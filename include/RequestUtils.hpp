...
typedef void (*unsubscribe_op_handler_t)(request *request, request_Unsubscribe *unSubscribeOp, response_OpResponse *opResponse);
...
class RequestUtils {
   private:
    response manage(request *request, send_op_handler_t send_op_handler, subscribe_op_handler_t subscribe_op_handler,
    ...
    void printUnSubscribeOperation(request_Unsubscribe *unSubscribe);
   ...
   public:
    ...
    request createRequest(const char *clientName, uint8_t fromMac[6], uint8_t toMac[6], int32_t messageType = 0);
    void buildSendOperation(request *request, const char *payload, const char *queue, bool retain = false, int32_t operationType = 0);
    void buildSubscribeOperation(request *request, const char *queue, int32_t operationType = 0);
    void buildPingOperation(request *request, int32_t ping, int32_t operationType = 0);
    response manage(const uint8_t *incomingData, int len,
                    ...
                    subscribe_op_handler_t subscribe_op_handler = subscribeOpHandlerDummy, unsubscribe_op_handler_t unsubscribe_op_handler = unSubscribeOpHandlerDummy,
